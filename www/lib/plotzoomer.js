...
/**
 * @classdesc PlotZoomer provides stacked zooming for a plot widget.
 */
...
class PlotZoomer extends PlotPicker {
	constructor(xAxis, yAxis, /*QWidget **/ canvas, doReplot = true) {
		 ...
		 * See also {@link PlotPicker}, {@link Plot#autoReplot autoReplot} and {@link Plot#replot replot}.
		*/
		 ...
		 * See also {@link setZoomBase} and {@link zoomRect}
		 */
		 ...
		 * See also {@link zoomRectIndex} and {@link scaleRect}
		 */
	...
	widgetKeyPressEvent(ke) {
		...
		if (!this.isActive()) {
			if (this.keyMatch(Enum.KeyPatternCode.KeyUndo, ke))
				this.zoom(-1);
			else if (this.keyMatch(Enum.KeyPatternCode.KeyRedo, ke))
				this.zoom(+1);
			else if (this.keyMatch(Enum.KeyPatternCode.KeyHome, ke))
				this.zoom(0);
		}
	...
	widgetMouseReleaseEvent(me) {
		const Enum = Enumerator.getDefaultEnumNampespace();
		if (this.mouseMatch(Enum.MousePatternCode.MouseSelect2, me))
			;//this.zoom(0);
		else if (this.mouseMatch(Enum.MousePatternCode.MouseSelect3, me))
			this.zoom(-1);
		else if (this.mouseMatch(Enum.MousePatternCode.MouseSelect6, me))
			this.zoom(+1);
		else
