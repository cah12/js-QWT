...
class Curve extends PlotSeriesItem {
    constructor(tle) {
        ...
        this.drawSeries = function (xMap, yMap, from, to) {
            ...
            if (Utility.mVerifyRange(numSamples, from, to) > 0) {
                ...
                if (m_symbol && (m_symbol.style() !== Symbol.Style.NoSymbol)) {
        ...
        this.drawSymbols = function (ctx, symbol, xMap, yMap, from, to) {
            ...
            mapper.setFlag(PointMapper.TransformationFlag.WeedOutPoints, this.testPaintAttribute(Curve.PaintAttribute.FilterPoints));
        ...
        this.drawSteps = function (painter, xMap, yMap, from, to) {
            ...
            if (c_attributes & Curve.CurveAttribute.Inverted)
                inverted = !inverted;
        ...
        this.legendIcon = function (index, size) {
            ...
            if (this.legendAttributes() == 0 || this.legendAttributes() & Curve.LegendAttribute.LegendShowBrush) {
                ...
                if (brush.color == Static.NoBrush && this.legendAttributes() == 0) {
                    ...
                    } else if (this.symbol() && (this.symbol().style() != Symbol.Style.NoSymbol)) {
            ...
            if (this.legendAttributes() & Curve.LegendAttribute.LegendShowSymbol) {
                if (this.symbol()) {
                    ...
                    if (this.symbol().style() == Symbol.Style.Ellipse)
                        sh -= 1;
        ...
        this.removePoint = function (point, always) {
            ...
            if (samples.length == 1 && !always) {
               Utility.alert("You cannot remove the only point in the curve. Remove the entire curve.")
                return;
            }
            var newSamples = [];
            //var x = point.x;
    ...
    drawLines(painter, xMap, yMap, from, to) {
        ...
        //const bool doAlign = QwtPainter::roundingAlignment( painter );
        var doFit = ((this.curveAttributes() & Curve.CurveAttribute.Fitted) && this.curveFitter());
...
Enumerator.enum("CurveStyle {  NoCurve = -1 , Lines , Sticks , Steps , Dots , UserCurve = 100  }", Curve);
Enumerator.enum("CurveAttribute { Inverted = 0x01 , Fitted = 0x02 }", Curve);
Enumerator.enum("LegendAttribute { LegendNoAttribute = 0x00 , LegendShowLine = 0x01 , LegendShowSymbol = 0x02 , LegendShowBrush = 0x04 }", Curve);
Enumerator.enum("PaintAttribute { ClipPolygons = 0x01 , FilterPoints = 0x02 , MinimizeMemory = 0x04 , ImageBuffer = 0x08 , FilterPointsAggressive = 0x10}", Curve);
